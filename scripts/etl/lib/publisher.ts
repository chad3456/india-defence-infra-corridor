...
export function publisherOf(
  source: Pick<FeedSource, "id" | "feed" | "discovery" | "publisherHost">,
): string {
  // A site-scoped search is that site, however it is reached.
  if (source.publisherHost) return source.publisherHost.replace(/^www\./, "");
  // Any other search reaches many publishers, but which ones is not knowable
...
/**
 * Whether a feed stands for a named newsroom.
 *
 * A site-scoped fallback does, however it is reached. An open keyword search
 * does not: it reaches many publishers, but which ones is not knowable until it
 * runs, so it cannot be counted towards a sector's publisher floor.
 */
export function namesAPublisher(
  source: Pick<FeedSource, "discovery" | "publisherHost">,
): boolean {
  return source.discovery !== true || Boolean(source.publisherHost);
}
